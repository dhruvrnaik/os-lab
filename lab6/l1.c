...
void fcfs(int count, int* executionTime) {
    char* pl[] = { "P1", "P2", "P3", "P4" };
    int ct[count];
    int i;
    int time = 0;
    for(i = 0; i < count; i++) {
        printf("%d-%s-", time, pl[i]);
        time += executionTime[i];
        printf("%d ", time);
        ct[i] = time;
    }
    int at[] = { 0, 0, 0, 0 };
    printResults(count, at, executionTime, ct, pl);
}

void npsjf(int count, int* arrivalTime, int* executionTime) {
    char* pl[] = { "P1", "P2", "P3", "P4" };
    bool pCompleted[] = { false, false, false, false };
    int completed = 0;
    int time = 0;
    int ct[count];
    while(completed != count) {
        ...
        int minIndex = -1;
        for(j = 0; j < count; j++) {
            if(!pCompleted[j] && arrivalTime[j] <= time && executionTime[j] < min) {
                min = executionTime[j];
                minIndex = j;
            }
        }
        ...
        if(minIndex != -1) {
            printf("%d-%s-", time, pl[minIndex]);
            pCompleted[minIndex] = true;
            time += executionTime[minIndex];
            ct[minIndex] = time;
            printf("%d ", time);
            completed++;
        } else {
...
int main() {
    const int count = 4;
    int arrivalTime[] = { 0, 2, 9, 2 };
    int executionTime[] = { 8, 4, 9, 15 };
    ...
    while(1) {
        ...
        switch(i) {
            ...
            case 3:
                ...
                break;
            case 4:
                fcfs(count, executionTime);
                break;
            case 5:
                npsjf(count, arrivalTime, executionTime);
                break;
            case 6:
                exit(0);
        }
